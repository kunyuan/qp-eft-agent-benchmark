...
## 5d. Long-horizon run protocol (relay + resume; learned from the L4 fable5 probes)

Multi-hour solver runs get interrupted (harness stream watchdogs, platform
incidents, usage limits). The protocol that makes interruptions minute-level
losses instead of session-level ones:

- **Agent-side discipline (now part of SETUP_L4 "Working protocol"):** the agent
  maintains `scratch/NOTES.md` (one factual line per substantive step) and
  externalizes derivations into `scratch/derivation_notes.md` as it goes. These
  double as the audit trail for `method.md`.
- **Maintainer-side resume (contamination-free):** relaunch with a FIXED template
  that only (1) states the interruption was infrastructural, (2) points the agent
  at its own `NOTES.md` / `derivation_notes.md`, and (3) restates the binding
  rules. NEVER paraphrase the agent's physics in a resume prompt — early L4
  probe-1 resumes contained steering phrases; every resume prompt must be
  archived verbatim in the run report for contamination accounting.
- **One attempt = the whole relay:** infra-killed sessions resumed from the
  agent's own artifacts count as a single attempt (see
  `.claude/memory/decisions.yaml` decision-001).
- **Harness knobs (Claude Code):** `CLAUDE_ASYNC_AGENT_STALL_TIMEOUT_MS`,
  `CLAUDE_STREAM_IDLE_TIMEOUT_MS`, `API_TIMEOUT_MS` govern the stall kills;
  lengthen for derivation-heavy runs. Keep the externalization discipline
  regardless — it is scientifically valuable on its own.
